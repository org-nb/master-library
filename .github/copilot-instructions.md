...
Extends the base instructions with everyday TypeScript coding conventions. Use Biome as the single tool for linting, formatting, and import sorting (`npx @biomejs/biome check --write .`). Do not configure or run ESLint, Prettier, or other separate linters and formatters. Ensure all Biome diagnostics pass cleanly before running `tara check` and staging changes. Use Python structure guidelines for Typescript. Use pnpm, and not npm, bun or yarn.
